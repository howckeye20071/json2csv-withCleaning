...
for num in range(10000,310001,10000):
    with open("D:/news/setn"+ str(num) +".json", "r", encoding='utf8') as jsonfile:
        data = json.load(jsonfile)
        with open("D:/news/csv/setn"+ str(num) +".csv", "w", encoding='utf8', newline="") as outfile:
            csv_out = csv.writer(outfile, delimiter=',')
            csv_out.writerow(
                ["title", "date", "author", "text", "url", "tags", "type_list", "source", "views", "share", "like"])
            # print(data.keys())
